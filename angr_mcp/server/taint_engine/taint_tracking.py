...
def remove_taint(dst, state):
    ...
    state.globals[UNTAINT_DATA][UNTAINTED_VARS].extend(map(str, leafs))
    deps = state.globals[GLOB_TAINT_DEP_KEY]
    pending = list(deps.items())
    seen_masters = state.globals[UNTAINT_DATA][SEEN_MASTERS]

    while pending:
        master, slaves = pending.pop(0)
        if master in seen_masters:
            continue

        untainted_vars = set(state.globals[UNTAINT_DATA][UNTAINTED_VARS])
        if set(master).issubset(untainted_vars):
            seen_masters.append(master)
            for entry in slaves:
...
class TaintTracker(ExplorationTechnique):
    ...
    def step(self, simgr, *kargs, **kwargs):
        ...
        if self._follow_unsat and len(simgr.unsat) > 0:
            for unsat_state in list(simgr.unsat):
                self._drop_constraints(unsat_state)
